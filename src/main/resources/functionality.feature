Feature: Functionality
  ...
  Scenario Outline: Check that User can shift to Amazon site in a specific country
    ...
    Then User checks that new web site Amazon logo contains country code '<countryCode>'
  ...
  Scenario Outline: Check that currency change working correctly
    ...
    Examples:
      | homePage                 | keyword       |actualTypeOfCurrency | newTypeOfCurrency  |
  ...
  Scenario Outline: Check that shipping location is correct
    ...
    Examples:
      | homePage                | keyword           | locationCountry   |
      | https://www.amazon.com/ | samsung galaxy    | South Africa      |


  Scenario Outline: Check that it is impossible to create new user account with empty email field
    Given User opens '<homePage>' page
    ...
    And User enter password '<password>' into Reenter Password field
